...
def transform_user_data(**kwargs):
    users_s3_key = kwargs['ti'].xcom_pull(task_ids='Extract_User_Json_to_S3')
    carts_s3_key = kwargs['ti'].xcom_pull(task_ids='Extract_Cart_Json_to_S3')
...
def transform_product_data(**kwargs):
    products_s3_key = kwargs['ti'].xcom_pull(task_ids='Extract_Product_Json_to_S3')
    carts_s3_key = kwargs['ti'].xcom_pull(task_ids='Extract_Cart_Json_to_S3')
...
def load_user_data(**kwargs):
    users_path = kwargs['ti'].xcom_pull(task_ids='Transform_User_Data')
    load_csv_to_rds(users_path, db_config, "users_table", "user_id")

def load_product_data(**kwargs):
    products_path = kwargs['ti'].xcom_pull(task_ids='Transform_Product_Data')
    load_csv_to_rds(products_path, db_config, "products_table", "product_id")

def load_cart_data(**kwargs):
    carts_path = kwargs['ti'].xcom_pull(task_ids='Transform_Cart_Data')
